...
# W2-4: ahora son DOS contratos (OhuVault + MutualPool); ambos WASMs deben bajarse a MVP.
WASMS=("$CONTRACTS/wasm/OhuVault.wasm" "$CONTRACTS/wasm/MutualPool.wasm")
...
for WASM in "${WASMS[@]}"; do
  [ -f "$WASM" ] || { echo "ERROR: falta $WASM (¿cargo odra build?)"; exit 1; }
  ...
  wasm-opt "$WASM" --disable-bulk-memory --disable-sign-ext -o /dev/null 2>/dev/null \
    && echo "    OK: $(basename "$WASM") MVP-limpio ($(wc -c < "$WASM") bytes)" \
    || { echo "ERROR: $(basename "$WASM") aún tiene ops bulk-memory/sign-ext"; exit 1; }
done
